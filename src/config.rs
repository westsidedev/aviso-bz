...
impl UserData {
    pub async fn load() -> UserData {
        let file = Path::new("config/avisobz/userdata.json");
    ...
    pub async fn create(email: &str, senha: &str, cookies: &str, proxy: &str) -> UserData {
        if Path::new("config/avisobz/userdata.json").exists() {
            let _ = fs::remove_dir_all("config/avisobz");
        }
        let path = "config/avisobz";
        let _ = fs::create_dir_all(path);
        let _ = fs::create_dir_all("config/avisobz/screenshot");
    ...
    pub async fn modify(email: &str, senha: &str, cookies: &str, proxy: &str) -> UserData {
        let path = "config/avisobz";
        let _ = fs::remove_file("config/avisobz/userdata.json");
    ...
    #[allow(dead_code)]
    pub async fn delete() -> () {
        fs::remove_dir_all("config/avisobz").unwrap();
    }
}
...
fn log_user(type_log: TypeLog, struct_name: &str, msg_log: &str) -> Result<(), std::io::Error> {
    let path = "config/avisobz/account.log";
